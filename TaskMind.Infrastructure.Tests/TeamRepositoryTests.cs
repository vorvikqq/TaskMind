...
namespace TaskMind.Infrastructure.Tests
{
    public class TeamRepositoryTests : IDisposable
    {
        ...
        public async Task CompleteWorkflow_CreateUpdateDelete_ShouldWorkCorrectly()
        {
            ...
            var exists = await _repository.ExistsAsync(createdTeam.Id);
            exists.Should().BeTrue();

            // Act - Update
            createdTeam.Name = "Updated Workflow Team";
            var updatedTeam = await _repository.UpdateAsync(createdTeam);

            // Assert - Update
            updatedTeam.Name.Should().Be("Updated Workflow Team");

            var retrievedTeam = await _repository.GetByIdAsync(createdTeam.Id);
            retrievedTeam!.Name.Should().Be("Updated Workflow Team");

            // Act - Delete
            await _repository.DeleteAsync(createdTeam.Id);

            // Assert - Delete
            var existsAfterDelete = await _repository.ExistsAsync(createdTeam.Id);
            existsAfterDelete.Should().BeFalse();

            var retrievedAfterDelete = await _repository.GetByIdAsync(createdTeam.Id);
            retrievedAfterDelete.Should().BeNull();
        }

        [Fact]
        public async Task TeamWithComplexRelationships_ShouldHandleCorrectly()
        {
            // Arrange
            var team = new Team { Name = "Complex Team" };
            await _context.Teams.AddAsync(team);
            await _context.SaveChangesAsync();

            // Add multiple employees and tasks
            var employees = new List<Employee>
        {
            new Employee { Name = "Dev 1", Skills = new List<string> { "C#", "React" }, TeamId = team.Id },
            new Employee { Name = "Dev 2", Skills = new List<string> { "Python", "SQL" }, TeamId = team.Id }
        };
            var tasks = new List<TaskItem>
        {
            new TaskItem { Title = "Feature A", TeamId = team.Id, Status = TaskState.InProgress },
            new TaskItem { Title = "Bug Fix B", TeamId = team.Id, Status = TaskState.New }
        };
            ...
            // Assert
            result.Should().NotBeNull();
            result!.Name.Should().Be("Complex Team");
            ...
            {
                emp.TeamId.Should().Be(team.Id);
            }
            foreach (var task in result.Tasks)
            {
                task.TeamId.Should().Be(team.Id);
            }
        }
